...
from config.utility import timer, selectionSort, time_block
import random
...
class Schedule: #hmm learning concpet diff between ref blueprint and inst obj if inst then will refer to inst? what if multiple
    ...
    def generate_schedule(self, dayTimeSlotsStandardizedStN, dayTimeSlotsStandardizedNtS, dayTimeSlotsKeysList, daysTasks, tasks_dict, task_manager, employee_manager): #would having lenght of day be predefined actually give notacible improv???? no 
        ...
        for qeue in self.dynamicTimeSlotQueuesDict:
            self.dynamicTimeSlotQueuesDict[qeue].assign_tasks(dayTimeSlotsStandardizedStN, dayTimeSlotsStandardizedNtS, task_manager, employee_manager)    
        #run all the queues until finished

    def generate_dynamic_time_slot_qeues_for_day(self, dayTimeSlotsKeysList, daysTasks, tasks_dict):
    ...
    class dynamicTimeSlotQueue:
        ...
        def __init__(self,time_slot_name, daysTasks):
            self.queue = []
            ...
            self.length = 0 #num tasks in queue #updated at qeue population
            self.windowed_tasks_list = []
             
        @timer
        def populate_queue(self, tasks_dict): #maybe change to generate later idk, semantics            
            
            @timer
            def searchStartTimePeriodTasks(daysTasks: list, searchVal: str):
            ...
            tasks_that_meet_start_time_criteria = searchStartTimePeriodTasks(self.daysTasks,self.time_slot)
            ...
            @timer
            def searchQuereDurationPeriodTasks(tasks_meet_prev_criteria: list):  
                tasks_meet_prev_criteria.sort(key=lambda task: tasks_dict[task].duration, reverse = True)
                return tasks_meet_prev_criteria
            ...
            def sortTierPeriodTasks(prev_sorted_tasks_local):
                prev_sorted_tasks_local.sort(key=lambda task: tasks_dict[task].tier)
                return prev_sorted_tasks_local
            
            tasks_sorted_tier = sortTierPeriodTasks(tasks_stage2_now_sorted_by_duration)
            
            #TODO this will prob need quite a bit of work
            def collect_windowed_tasks(tasks: list): #NOTE shortcoming - wut if optinal tasks have greater duration???????? if duration longer than optinal window then no longer an optional task?, really less optional task & more like windowedTask
            ...
            tasks_sorted_final = collect_windowed_tasks(tasks_sorted_tier)
        ...
        def assign_tasks(self, dayTimeSlotsStandardizedStN, dayTimeSlotsStandardizedNtS, task_manager, employee_manager, employee_info=None):
            ...
            def assign_task(task_name):
                ...
                def assign_employees_to_task(employees_with_req_traits_local, task_name, time_slot):
                    ...
                    def choose1():
                        ...
                        #TODO can later replace with prob logic
...
@timer 
def instantiate_and_run_scheduler(dayTimeSlotsStandardizedStN, dayTimeSlotsStandardizedNtS, dayTimeSlotsKeysList, daysTasks, tasks_dict, task_manager, employee_manager):
    schedule = Schedule()
    schedule.generate_schedule(dayTimeSlotsStandardizedStN, dayTimeSlotsStandardizedNtS, dayTimeSlotsKeysList, daysTasks, tasks_dict, task_manager, employee_manager)

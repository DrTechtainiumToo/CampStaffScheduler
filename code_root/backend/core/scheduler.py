...
from code_root.backend.core.employees import EmployeeManager, Employee
import logging
...
class Schedule:
    ...
    #TODO im confused between days_tasks and tasks_dict, cleanup meaning
    #days task is just [tasks dict]
    #sort this out, we need more clear var names 
    ...
    def generate_schedule(self,
        ...
        for queue in self.time_slot_task_queues:
            self.time_slot_task_queues[queue].assign_tasks_in_period(
    ...
    class DynamicTimeSlotQueue:
        ...
        def populate_queue(self, tasks_dict: dict[str, TaskManager.Task]):  # maybe change to generate later idk, semantics
            ...
            current: int = Schedule.DynamicTimeSlotQueue.static_task_count_total
            Schedule.DynamicTimeSlotQueue.static_task_count_total = current + len(list(queue - windowed_tasks))
        ...
        def _assign_employees_to_task(
            ...
            #Schedule.DynamicTimeSlotQueue.static_task_count_total
        ...
        def _assign_single_task(
            ...
            if task_name in self.windowed_tasks_list:
                ...
                if current_assignments < required_assignments:
                    ...
                    schedule.time_slot_task_queues[next_queue_key].windowed_tasks_list.append(task_name) #BUG not referencing schedule properly
        ...
        def _roll_over_tasks(self, task: str, index_to_time_slot_map: dict[int, str], schedule) -> None:
                ...
                if avail_employees_in_period: 
                    ...
                    if ppl_needed.isdigit():
                        if (len(avail_employees_in_period)+1 >= int(ppl_needed)): 
                            ...
                            self._roll_over_tasks(task, index_to_time_slot_map, schedule)
                        else: #Non windowed task just get ignored, goes to
                            self._roll_over_tasks(task, index_to_time_slot_map, schedule) #make sure rest gets captured, before break
                            break
                    else:
